...
class Matrix:
    ...
    def __init__(self, d: tuple, values: list[list[float]]) -> None:
        self.__dimensionCheck(d, values)
        self.d = d
        self.values = values
    ...
    def multiply(self, m, logging=False):
        ...
        d = (self.d[0], m.d[1])
        values, temp = [], []
        vecArrA = [Vector(self.d[1], self.values[i]) for i in range(self.d[0])]
        vecArrB = [Vector(self.d[1], [row[i] for row in m.values]) for i in range(m.d[1])]
    ...
    def inverse(self, logging=False):
        ...
        cofactorMatrix = Matrix(self.d, cofactorArr)
    ...
    def __dimensionCheck(self, d: tuple, values: list[list[float]]) -> None:
        ...
        if (len(d) != 2) or (d[0] < 1 and d[1] < 1):
            raise ValueError(Fore.RED + "Invalid dimensions for the matrix" + Style.RESET_ALL)

        if len(values) != d[0]:
            raise ValueError(Fore.RED + "Number of row's inputted don't match the specified dimensions" + Style.RESET_ALL)
        
        if all(len(row) == d[1] for row in values) != True:
            raise ValueError(Fore.RED + "Number of columns's inputted don't match the specified dimensions" + Style.RESET_ALL)
...
def main():
    matA = Matrix((4,4), [[2,1,3,0],[1,2,1,3],[3,1,2,2],[5,2,7,3]])
    ...
    matC = Matrix((3,1), [[1],[2],[3]])
    matC.toVector().normalize(logging=True)
    matD = Matrix((2,2), [[2,0],[0,2]])
